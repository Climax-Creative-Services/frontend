import React from "react";
import { useState } from "react";
import { Link, NavLink } from "react-router-dom";
import classes from "./Navbar.module.css";

const Navbar = () => {
  const [isClicked, setIsClicked] = useState(false);

  const menuToggle = () => {
    setIsClicked(!isClicked);
    const toggleMenu = document.querySelector(".menu");
    toggleMenu.classList.toggle("active");
  };
  return (
    <div className={classes.main}>
      <div className={classes.fistNavbar}>
        <p className={classes.iconText}>CLIMAX</p>
        <div className={classes.navbarList}>
          <ul className={classes.navbarMenu}>
            <li className={classes["link-line"]}>Learn</li>
            <li className={classes["link-line"]}>Hub</li>
            <li className={classes["link-line"]}>Market</li>
            <li className={classes["link-line"]}>Learn</li>
          </ul>
        </div>
        <form className={classes["search-container"]}>
          <label htmlFor="search-box">
            <img
              src="data:image/svg+xml;base64,"
              alt="search"
              about="search"
              className={classes.searchIcon}
            />
          </label>
          <input
            id="search-box"
            placeholder="Search..."
            type="text"
            className={classes["search-box"]}
            name="search"
          />
        </form>

        <div className={classes.userProfile}>
          <div className={classes.action}>
            <div className={classes.profile} onClick={menuToggle}>
              <img
                src="https://pbs.twimg.com/profile_images/1503410280642064386/gfH8F4HQ_400x400.jpg"
                alt="profile"
              />
            </div>
            <div className={`${classes.menu} ${isClicked && classes.active}`}>
              <h3>Ekin Köseoğlu</h3>
              <span>Website Designer</span>
              <ul>
                <li>
                  <img src="icons/user-icon.png" /> <a href="#">My Profile</a>
                </li>
                <li>
                  <img src="icons/edit-icon.png" /> <a href="#">Edit</a>
                </li>{" "}
                <li>
                  <img src="icons/inbox-icon.png" /> <a href="#">Inbox</a>
                </li>{" "}
                <li>
                  <img src="icons/settings-icon.png" /> <a href="#">Settings</a>
                </li>{" "}
                <li>
                  <img src="icons/help-icon.png" /> <a href="#">Help</a>
                </li>
                <li>
                  <img src="icons/logout-icon.png" />
                  <NavLink to="/login">Logout</NavLink>
                </li>
              </ul>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Navbar;
